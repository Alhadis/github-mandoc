	...
	module Mandoc
		...
		class Rendering
			...
			def filter(source, path = "")
				...
				@doc.css(".Nd, .permalink, .selflink, table + br").each { |el| unwrap el }

				fix_implicit_paragraphs
				fix_synopses
				fix_anchors
				replace_unwhitelisted_tags
				fix_double_nested_tags %w[code b]
				...
				# Finally, strip anything doomed to be filtered by GitHub's HTML sanitiser
				%w[class style title].each { |attr| @doc.css("[#{attr}]").remove_attr(attr) }
				@doc.css("section[id]").each { |el| el.name = "div" }
				@doc.css("section").each { |el| unwrap el }
				gsub %r{\s*<div(?!.*?\sid=")[^>]*>\s*</div\s*>\s*}i, ""
			end
			...
			def fix_displays
				...
				return if (indented = @doc.css("div.Bd-indent")).empty?

				indented.wrap "<ul></ul>"
			end

			# Remove tags that pointlessly enclose another of the same type
			def fix_double_nested_tags(names)
				names.each do |name|
					until (nested = @doc.css("#{name} #{name}")).empty?
			...
			def normalise_whitespace(element)
				return element unless element.ancestors("pre").empty?
